...
# Build the project (skipped - using pre-built binary)
build_project() {
    print_step "Using pre-built collector binary"

    if [ ! -f "$PROJECT_ROOT/target/release/collector" ]; then
        print_error "Binary not found at target/release/collector. Please build first with: cargo build --release --package collector"
        exit 1
    fi

    print_step "Binary found and ready to install"
}
